(() => {
  ...
  const VERSION = 'panel-character-v2';
  ...
  function renderPortalsPanel() { ui.panelBody.innerHTML = `<div class="panel-topline"><div class="panel-section-title">Portals</div><button class="mini-btn" id="addPortalBtn">+ Portal</button></div><div id="portalEditorList" class="portal-editor-list"></div><button class="panel-action danger-action" id="resetQuestionsBtn">Reset edited portals</button>`; document.getElementById('addPortalBtn').onclick = addPortal; document.getElementById('resetQuestionsBtn').onclick = resetTopics; const list = document.getElementById('portalEditorList'); topics.forEach((t, ti) => { const d = document.createElement('details'); d.className = 'portal-details'; d.innerHTML = `<summary><span><strong>${esc(t.title)}</strong><small>${t.questions.length} questions • ${topicSolved(ti)} solved • ${topicWrong(ti)} wrong</small></span><span class="summary-caret">⌄</span></summary><div class="portal-tools"><button class="mini-btn" data-a="editp">Edit portal</button><button class="mini-btn" data-a="addq">+ Question</button><button class="mini-btn" data-a="play">Play</button></div><div class="question-list"></div>`; d.querySelector('[data-a="editp"]').onclick = () => editPortal(ti); d.querySelector('[data-a="addq"]').onclick = () => editQuestion(ti, -1); d.querySelector('[data-a="play"]').onclick = () => openPortal(ti); const ql = d.querySelector('.question-list'); t.questions.forEach((q, qi) => { const b = document.createElement('button'); b.className = 'question-row'; b.innerHTML = `<span>${qi + 1}. ${esc(short(q.question || 'Empty question', 58))}</span><em>${status(qid(ti, qi))}</em>`; b.onclick = () => editQuestion(ti, qi); ql.appendChild(b); }); list.appendChild(d); }); }
